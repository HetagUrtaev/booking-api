...
class RoomsFacilitiesRepositories(BaseRepositories):
    ...
    async def set_rooms_facilities(self, room_id: int, facilities_ids: list[int]):
        get_corrent_facilities_ids_query = select(self.model.facility_id).filter_by(
            room_id=room_id
        )
        ...
        ids_to_insert = list(
            set(facilities_ids) - set(corrent_facilities_ids)
        )  # список услуг которые нужно добавить в бд
        ids_to_delete = list(
            set(corrent_facilities_ids) - set(facilities_ids)
        )  # список услуг которые нужно удалить из бд
